...
## Phase 7 — Cross-inject re-map (in progress)
ARCHITECTURE NOTE: J/I cross-inject content is generated by HARDCODED strings in
report_builder's _legacyDerivedSectionF* functions (keyed off source-screen
answers), NOT from the phrase bank. This is a mature, heavily-tested subsystem
(58 report_builder tests) that currently produces J1 Building, J2 Grounds, J3
People, J4 Health, J5 Security. The bank keys added in Phases 5-6 (J2/J3/I2) are
reference content; the report uses the derived strings. Phase 7 sequenced
safest-first; the J1-J5 -> J1-J4 structural renumber (fold Health->People,
Security->Other) is a separate, high-risk step (rewrites assembly + ~58 tests).

- 2026-09-25 — 7.1 Spray-foam guarantee cross-inject wired. Added a block in
  _legacyDerivedSectionFIssueGuarantees firing when cb_spray_foam (F1 About Roof
