...
- **Full test suite** (`flutter test`): **1429 passed, 5 failed**. The 5 failures
  are the documented **pre-existing** failures unrelated to this delta
  (4 × `email_compose_sheet_test`, 1 × `inspection_phrase_engine_property_extended_test`);
  they fail identically on `main`. **Zero new regressions.**
...
- **Pre-existing test failures (5):** not introduced by this delta; recommend a
  separate fix (email compose sheet widget + one roof-material engine test).
- **J2/J3 reference keys:** the revised J2 (grounds) and J3 (people) risk
  statements were added to the approved bank as reference content. The report's
  J2/J3 output is generated by the report-assembly cross-inject functions (now
  aligned to the revised structure); the reference keys give a single approved
  source should those be migrated to bank-driven rendering later.
