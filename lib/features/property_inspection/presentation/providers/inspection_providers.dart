...
class InspectionScreenNotifier extends StateNotifier<InspectionScreenState> {
  ...
  String _buildAutoGeneratedPhraseText(
    InspectionNodeDefinition definition,
    Map<String, String> answers,
  ) {
    final phraseEngine = _ref.read(inspectionPhraseEngineProvider);
    final enginePhrases =
        phraseEngine?.buildPhrases(_screenId, answers) ?? const <String>[];
    final fieldPhrases =
        FieldPhraseProcessor.buildFieldPhrases(definition.fields, answers);
    return [...enginePhrases, ...fieldPhrases].join('\n\n');
  }

  bool _shouldRestorePersistedPhraseText({
    required String screenId,
    required List<String> persistedPhrases,
    required bool isManualEdit,
    required String autoGeneratedText,
  }) {
    if (persistedPhrases.isEmpty) return false;
    if (isManualEdit) return true;

    final persistedText = persistedPhrases.join('\n\n').trim();
    if (persistedText.isEmpty) return false;
    if (persistedText == autoGeneratedText.trim()) return false;
    if (_isKnownStaleAutoPhrase(screenId, persistedPhrases)) return false;

    // Compatibility fallback for rows created before the manual-edit flag
    // existed. If the cached text differs from the current auto-generated
    // preview and is not one of the known stale auto-cache patterns, keep it.
    return true;
  }

  bool _isKnownStaleAutoPhrase(
    String screenId,
    List<String> persistedPhrases,
  ) {
    final normalizedId = screenId.trim().toLowerCase();
    final joined = persistedPhrases.join(' ').trim().toLowerCase();
    ...
    if (normalizedId == 'activity_in_side_property_fire_places__other') {
      return joined.contains('incorporates a .');
    }
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final autoGeneratedText = screenDefinition == null
          ? ''
          : _buildAutoGeneratedPhraseText(screenDefinition, answers);

      // Only restore persisted phrase text as edited preview when it was
      // manually edited, or when an older pre-flag row clearly differs from
      // the current auto-generated output and is not one of the known stale
      // auto-cache patterns.
      ...
      if (meta?.phraseOutput != null && meta!.phraseOutput!.isNotEmpty) {
        try {
          ...
          if (_shouldRestorePersistedPhraseText(
            screenId: _screenId,
            persistedPhrases: phrases,
            isManualEdit: meta.phraseEditedManually,
  ...
  Future<void> _persistPhraseOutput() async {
    ...
    try {
      ...
      final isManualEdit = state.editedPhraseText != null;
      ...
      await _repo.savePhraseOutput(
        ...
        phraseEditedManually: isManualEdit,
      );
